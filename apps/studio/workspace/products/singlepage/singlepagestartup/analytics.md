...
| Segment and Sales reference                                             | Observation  | Period                       | Source and limitation                                         |
| ----------------------------------------------------------------------- | ------------ | ---------------------------- | ------------------------------------------------------------- |
| `makers`: `hosted-chat`, `video`, `search-community`                    | Not measured | No reporting window supplied | No attributable acquisition dataset is connected.             |
| `makers`: `reached-explanation` → `adoption-decided`                    | Not measured | No reporting window supplied | Evaluation and adoption events are unavailable.               |
| `developer-agents`: `human-referral`, `task-discovery`, `project-reuse` | Not measured | No reporting window supplied | Agent recommendation and reuse are not observed here.         |
| `developer-agents`: `assigned-need` → `reuse-recommended`               | Not measured | No reporting window supplied | No comparable project cohort or outcome dataset is available. |

## Product usage and retention

Repository visits, qualified starts, completed server deployments, adopted modules, successful project adaptations, repeated reuse, support requests and contributions cannot yet be reported. Future observations must separate a repository view, clone or generated project from confirmed use in a product. Technical test results may support implementation quality, but they do not establish customer adoption or business value.
