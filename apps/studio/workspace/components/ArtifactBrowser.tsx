...
function reviewGuidance(kind: IStudioArtifact["kind"]): {
  frequency: string;
  impact: string;
} {
  if (["brief", "business", "research", "evidence"].includes(kind)) {
    return {
      frequency:
        kind === "evidence"
          ? "updated as facts change"
          : "rarely changes after approval",
      impact: "errors propagate to every later stage",
    };
  }
  if (kind === "strategy") {
    return {
      frequency: "changes when the commercial direction changes",
      impact: "affects Brand, Design, and every Product",
    };
  }
  if (kind === "brand") {
    return {
      frequency: "rarely changes after approval",
      impact: "affects meaning and communication across every Product",
    };
  }
  return {
    frequency: "changes as the visual system evolves",
    impact: "affects visual execution across every Product",
  };
}
...
export function ArtifactDocument({
  ...
  const projectionDescription =
    workspace.id === "default"
      ? "Studio shows the resolved default document. Edit the canonical source files listed below; this projection is read-only."
      : "Studio shows one source layer for comparison. Edit the canonical source file listed below; this projection is read-only.";
  const guidance = reviewGuidance(artifact.kind);
  return (
