...
Runnable studio are standalone imported prototypes. They are not the reusable
SPS design-system catalog. Reusable presentation belongs in Storybook under
`apps/studio/modules/<module>/<models|relations>/<entity>/<layer>/<block-id>`.
...
- Its reusable SPS UI has already been migrated to Storybook or replaced by
  existing module-owned Storybook studio.
...
- `npm run studio:validate` continues to pass.
