...
get_last_env() {
    get_env "${BASH_SOURCE[0]}" "$1" | tail -n 1 | tr -d '\r'
}

API_SERVICE_URL_FROM_ENV=$(get_last_env "API_SERVICE_URL")
API_URL="${API_SERVICE_URL_FROM_ENV:-http://localhost:4000}"
CURL_CONNECT_TIMEOUT="${CURL_CONNECT_TIMEOUT:-10}"
CURL_MAX_TIME="${CURL_MAX_TIME:-60}"

RBAC_SUBJECT_IDENTITY_EMAIL=$(get_last_env "RBAC_SUBJECT_IDENTITY_EMAIL")
RBAC_SUBJECT_IDENTITY_PASSWORD=$(get_last_env "RBAC_SUBJECT_IDENTITY_PASSWORD")
RBAC_SECRET_KEY=$(get_last_env "RBAC_SECRET_KEY")
