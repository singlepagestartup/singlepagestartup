...
- Backend HTTP cache (`libs/middlewares/src/lib/http-cache/index.ts`): GET responses are cached in KV with versioned exact-path keys and TTL `KV_TTL` (default 30 s; production sets `KV_TTL=43200` (12 h) and `MIDDLEWARE_HTTP_CACHE=true` in `apps/api/.env.production:19,21`). Successful 2xx mutations bump cache versions in a fire-and-forget async block (`:133-213`) for the mutation path, the path without a trailing UUID, and `/rbac/permissions`. The generic factory client sends no `Cache-Control` header (0 occurrences in `libs/shared/frontend/client/api`), but the chat timeline components DO pass `"Cache-Control": "no-store"` explicitly (`…/message/list/default/client.tsx:32,54`, `server.tsx:28,50`, and other chat fetches), so chat timeline GETs bypass the HTTP cache today. The middleware already carries an issue-152 exclusion for cart aggregate counters with the comment "exact-path cache invalidation may leave stale quantity/total after cart mutations" (`:94-103`) — a documented precedent of this staleness class.
- Middleware registration order (`apps/api/app.ts:153-168`): `HTTPCacheMiddleware` is registered (conditionally, behind its feature flag) BEFORE `RevalidationMiddleware`. Hono unwinds post-`next()` code inner-first, so the revalidation WS broadcast currently runs BEFORE the http-cache version bump. Any "await the bump" change inside http-cache alone cannot fix broadcast-outruns-bump — the registration order itself must change.
...
- HTTP-cache granularity is safe for the generic factory but NOT for chat paths: factory base routes bump correctly (`/api/<module>/<models>/<id>` minus UUID = the list GET path), but chat message update/delete hit chat-scoped paths (`…/chats/<cid>/messages/<mid>`) and bump only `…/chats/<cid>/messages` — the thread-scoped timeline GET (`…/chats/<cid>/threads/<tid>/messages`) is never bumped, and the backend cannot derive it from the mutation URL (`threadId` is absent). Today this is mitigated client-side: chat timeline fetches pass `"Cache-Control": "no-store"`, so the timeline GET is never served from the HTTP cache. The Phase 4 server-side exclusion is defense-in-depth, not a live-bug fix: it removes the dependency on every present and future chat fetch remembering the header — critical in production where `KV_TTL=43200` (12 h) would make a single forgotten header a severe stale-data bug.
...
- `appendToListQueries(queryClient, route, item, opts?)` — uses `queryClient.setQueriesData({ queryKey: [route] }, updater)` for prefix coverage of `[route]` and `[route, serializedParams]` keys. Safety rules: deduplicate by `id` (if the item already exists, patch it instead); append ONLY into list queries with no meaningful params (second key element absent/empty). Any query whose serialized params carry `filters`, `orderBy`/`sort`, `limit`/`offset`/pagination, or `search` must NOT receive an append — the new item may not match the filter, belongs at an unknown sort position, or would break page boundaries; instead immediately invalidate that specific query (targeted, not deferred). In exact-key mode (explicit query key provided by the caller, e.g. chat), append is allowed because the caller owns the key semantics. Must also accept an explicit full query key (for hand-written SDKs with one-element URL keys).
...
**Changes**: Follow the `request-limmiter/index.spec.ts` BDD format (top-level `BDD Suite` JSDoc with Given/When/Then, per-test `BDD Scenario`). Cover: append dedup-by-id; append into param-bearing queries (filters, orderBy, limit/pagination, search — each as its own scenario) → targeted invalidation, not append; append into param-less queries succeeds; exact-key mode append; patch no-op on absent id; remove preserves references of unmatched queries; structural sharing (unchanged items keep identity); prefix vs exact key modes; factory create/update/delete run internal cache handling AND the user-provided `reactQueryOptions.onSuccess` afterwards, in that order.
...
**Why**: Chat message update/delete mutations hit chat-scoped paths and bump only `…/chats/<cid>/messages`; the thread-scoped timeline GET (`…/chats/<cid>/threads/<tid>/messages`) is never version-bumped and cannot be derived from the mutation URL (no `threadId`). Today this is mitigated only client-side: chat timeline fetches pass `"Cache-Control": "no-store"` (`…/message/list/default/client.tsx:32,54` and the chat server components), so there is NO live stale-data bug. The exclusion is defense-in-depth: it moves the guarantee server-side so no present or future chat fetch can reintroduce the staleness class by forgetting the header — which in production (`KV_TTL=43200`, 12 h, `MIDDLEWARE_HTTP_CACHE=true`) would be a severe bug of the kind already documented by the issue-152 cart-counter exclusion (`:94-103`).
**Changes**: Exclude the chat thread-messages GET path (`/api/rbac/subjects/<id>/social-module/profiles/<pid>/chats/<cid>/threads/<tid>/messages`) from HTTP caching, following the issue-152 exclusion precedent (path-pattern early `next()` before the cache lookup). Realtime chat data and a long-TTL server-side cache are fundamentally at odds, so exclusion is preferred over inventing a mutation→read-path bump rule the middleware cannot compute. Also evaluate the chat actions GET path (`…/chats/<cid>/actions`): actions are created server-side (no HTTP mutation passes through this middleware), so its cache version is never bumped either — apply the same exclusion if its fetches are not uniformly `no-store`. Add a comment in the middleware referencing this issue, mirroring the issue-152 comment style. This item is hardening, not a blocker: it must not change observable behavior while clients keep sending `no-store`.
...
**Files**: `apps/api/app.ts` (`:153-168`), `libs/middlewares/src/lib/http-cache/index.ts` (`:133-213`)
**Why**: Two independent defects break bump-before-broadcast ordering today. (1) Registration order: `HTTPCacheMiddleware` is registered before `RevalidationMiddleware` (`app.ts:154` vs `:168`); Hono unwinds post-`next()` code inner-first, so the revalidation broadcast fires BEFORE http-cache's post-response block even runs — awaiting bumps inside http-cache alone cannot fix this. (2) The bump itself is fire-and-forget, so even with correct ordering the middleware chain does not wait for it. With the 1000 ms client delay gone, another client's immediate refetch can outrun the bump and receive a stale cached response (catastrophic in production with `KV_TTL=43200`).
**Changes**: Fix both: (a) reorder registration in `apps/api/app.ts` so `RevalidationMiddleware` is registered BEFORE the (feature-flag-conditional) `HTTPCacheMiddleware` block — making http-cache inner, so its post-response bump unwinds before the revalidation broadcast; keep the http-cache registration inside its existing conditional. (b) Await the cache-version bumps for mutation methods (POST/PUT/PATCH/DELETE) before the http-cache middleware completes. Verify the swapped order does not regress GET cache-hit early returns (revalidation ignores GETs, so none expected) or the other middlewares between them (`ActionLogger`, `IsAuthorized`, `BillRoute` — confirm none depends on running inside/outside http-cache). Document the ordering contract with comments in `app.ts` and both middleware files. GET response-caching writes may remain fire-and-forget.
...
- Cache helpers: append dedup, param-bearing append (filters / orderBy / limit / pagination / search) → targeted invalidation, param-less and exact-key append success, patch/remove no-ops, structural sharing/reference stability, prefix vs exact key modes, non-array cache tolerance.
...
1. Start infra + apps (`./up.sh`, `npm run api:dev`, `npm run host:dev`) — research noted `localhost:4000` was down during the research browser check, so end-to-end WS behavior MUST be verified during implementation.
   - Prerequisite: a working chat page. The seeded chat URL (`/en/social/chats/7f4540f7-eec6-42ac-be2d-836e5b1b0407`) currently renders `Page not found` even though the host shell and the `revalidation` component mount. Before browser verification, establish an authenticated context with seeded content where the chat overview and timeline actually render (sign in, verify profile/chat membership, or adjust seeds); without this, the chat flicker/rerender criteria cannot be checked manually.
...
- HTTP-cache version bumps are currently fire-and-forget AND ordered after the revalidation broadcast (registration order in `app.ts:153-168` + Hono inner-first unwinding); Phase 5 fixes both — middleware reorder plus awaited mutation-path bumps — so the WS broadcast cannot outrun backend cache invalidation. The added mutation latency is a few Redis `incr` round trips (~ms).
