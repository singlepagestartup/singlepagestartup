...
- `verifyJwt` takes an expected type. The is-authorized service, `init`, `me`, `logout` and OAuth `start` expect `access`; `refresh` expects `refresh`. A token without `typ` is accepted as either type until it expires.
...
- No change to the session cookie's `httpOnly` flag (SEC-13, N-03) or to the HTTP cache's position (N-04).
- No change to `apps/mcp`: it reads `subject.id` from subject tokens and signs its own tokens with its own claims.
- No change to the twelve inline ownership checks and the ownership middleware (they sit behind the is-authorized middleware), to the actions logger's attribution, or to the bill-route service's own verification and cache.
...
#### 2. refresh, init and OAuth start

**File**: `subject/api/service/singlepage/refresh.ts`, `init.ts`, `authentication/oauth/start.ts`
**Why**: each turns a presented token into acting as its subject.
**Changes**: `refresh` verifies `type: "refresh"` and refuses a revoked token after reading the subject. `init` verifies `type: "access"` and treats a revoked token like an expired one (new subject). `start` verifies `type: "access"`, reads the subject through its repository, and ignores a revoked token as it ignores an invalid one.
...
- [x] `npx nx run @sps/rbac:jest:test` and `npx nx run @sps/middlewares:jest:test` pass with new scenarios: refresh token refused by is-authorized, access token refused by refresh, untyped token accepted by both, revoked token refused by is-authorized, refresh, `me` and OAuth start, `init` not reusing a revoked or refresh token, logout writing `tokensValidAfter` only for a valid token, `me` reading the database row, the middleware not trusting a cached decision for any token of a subject after it logged out.
