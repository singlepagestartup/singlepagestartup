...
Every file the API serves under `/public/*` now carries `X-Content-Type-Options: nosniff` and `Content-Security-Policy: sandbox`. A file opened as a document gets an opaque origin with scripts disabled, while images, video, audio and CSS backgrounds that embed it load as before. Uploads are bounded by `FILE_STORAGE_MAX_UPLOAD_BYTES` (50 MiB by default), and an upload request carries one file. SVG uploads stay allowed; there is no content-type allow-list.
...
- `libs/shared/backend/utils` error mapping: Hono's `Payload Too Large` is a 400 validation error, so the chunked path answers 400 rather than 500.
...
- Documentation: `libs/modules/file-storage/README.md` (one file per request, the limit, the delivery headers, the providers) and `tools/deployer/README.md` (file storage in production).

## Verification

- [x] Unit lanes: `NX_DAEMON=false NX_ISOLATE_PLUGINS=false npx nx run-many --target=jest:test --projects=@sps/file-storage,@sps/backend-utils,@sps/shared-utils,api` (file-storage 22, backend-utils 127, shared-utils 74, api 4 tests)
...
- [x] Mutation checks: without the headers the scenario fails; without the file count the three multi-file specs fail; a pass-through middleware fails the five over-limit specs; reading the fetched body without the limit fails the two `create-from-url` specs; without the new keyword the chunked-upload spec and its error-mapping case fail
- [x] `curl` against the API: `curl -I` on an uploaded SVG shows both headers; a 51 MiB upload answers 400 with 0 bytes sent, the same upload chunked 400, `create-from-url` of it 400; two files 400; with `FILE_STORAGE_MAX_UPLOAD_BYTES=4096` an 8.5 KB file answers 400 and a 111-byte SVG 201
...
_Verify:_ upload a file under the limit (201), one over it (400) and two files in one request (400), then `curl -I` the stored `/public` URL and expect `X-Content-Type-Options: nosniff` and `Content-Security-Policy: sandbox`.
