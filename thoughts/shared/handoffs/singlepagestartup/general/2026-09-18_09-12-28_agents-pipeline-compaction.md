...
## Update 2026-09-18, phase 3

- PR #248 was merged (`80bf5cdb14`) with the owner's authorization; phase 3 is PR #250 on branch `claude/agents-pipeline-prose`, four commits, each with validated `Downstream-*` trailers, description in `thoughts/shared/prs/250_description.md`.
- The four decisions were taken: contracts removed without stubs; `flakecode/m2commerce` (local checkout under `/Users/rogwild/code/flakecode/`, `default_layer: startup`) is the only live downstream project with a Studio workspace and its workspace may be copied for read-only dry runs; `.agents/migrations/` confirmed; all eight migration procedures removed because no live workspace carries a legacy shape.
- The ledger's `Decisions taken` and `Phase 3 outcome` sections and the TSV `phase3` column record where every section's rules ended; `2026-09-18-pre-development-goldens-phase3.txt` is the baseline for phases 4 and 5 (8 template hash lines differ from phase 1, everything else identical).
- Templates were left byte-identical except four reference fixes; their comment trimming, the semantic rewrite of `structure.test.ts`, the editorial-pass heading requirement and the duplicate-sentence lint are phase 4. Phase 5 dry runs can use the m2commerce copy: link `.agents` into a scratch root beside a copy of `apps/studio/workspace` and run the check with `--repository-root <root> --repository flakecode/m2commerce`.
- `tools/studio/products/MIGRATION.md` is still in Russian and documents the retired Business transfer; translating or retiring it is an open decision for the owner.
