...
- Route billing returning a fixed subject instead of delegating to `getSubjectId`: its refused-token scenario fails.

Route billing was added after the first push: the direct `POST /api/rbac/subjects/authentication/bill-route` is allow-listed and resolved its subject with plain `jwt.verify` and its own token cache, so it would have charged a refresh or revoked token. `BillRouteService` now injects the is-authorized service and resolves the subject through its public `getSubjectId`.
...
- HTTP check of route billing (API restarted on 4311), direct `POST /api/rbac/subjects/authentication/bill-route` for the OpenRouter reply route: live access token 400 (the throwaway subject has no currency balance, so the subject was resolved on a billable route), refresh token 401, the same access token after logout 401; subject deleted (200); server stopped.
- After the billing change: `npx nx run @sps/rbac:jest:test` 87 suites / 411 tests.
