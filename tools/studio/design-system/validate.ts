...
async function validateFigmaMetadataPair(
  ...
  options: {
    ...
    figma: StudioFigmaManifest | undefined;
    source: StudioFigmaSource;
  },
): Promise<void> {
  ...
  let figmaJson: StudioFigmaJson;

  try {
    figmaJson = (await readJson(figmaPath)) as StudioFigmaJson;
  } catch (error) {
    addError(
      failures,
      manifestPath,
  ...
  const expectedMetadata: Record<string, unknown> = {
    "singlepagestartup.studio.blockId": expectedBlockId,
    "singlepagestartup.studio.layer": layer,
    "singlepagestartup.studio.syncKey": syncKey,
...
async function main(): Promise<void> {
  ...
  console.log("Studios design system validation OK.");
}
