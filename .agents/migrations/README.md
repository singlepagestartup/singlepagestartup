...
| `standalone-business`     | `business/<layer>.md` exists or the index has an entry of kind `business`                                                                          | Retiring standalone Business                              |
...
The Business-to-model transfer maps for `standalone-business` and `catalog-v1`
were retired on the same grounds; the current shape is defined by
`.agents/templates/products.yaml`, `.agents/templates/product-model.md` and
`.agents/contracts/inheritance.md`. Read them when a detector reports one of
those shapes:

```bash
git show 818f097d229eadc9b839d7dd8538ddf66bfc07e4:tools/studio/products/MIGRATION.md
```
