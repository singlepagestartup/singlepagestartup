...
The detection catalogue behind `.agents/contracts/editorial-pass.md`. The
contract states the rules and the completion check; this file lists the shapes
that break them, with a rewrite for each. Load it while editing prose, not while
reading a role.

The examples are in English, and the principles apply by function rather than by
literal phrase. Preserve the grammar, punctuation, register, idiom, and sentence
rhythm of the requested language, and do not make non-English text sound
