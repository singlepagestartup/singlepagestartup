...
Follow `.agents/contracts/engineering/knowledge-first.md`: the approved research document is the primary source for this phase. Use its `file:line` references directly and spot-check only the claims the plan depends on; spawn new investigation sub-tasks only for questions the research does not answer.
