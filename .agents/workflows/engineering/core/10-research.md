...
Follow `.agents/contracts/engineering/knowledge-first.md`: consult the process log, ticket, and existing research/plan artifacts (for this issue and related topics) before spawning any codebase-wide investigation. If a research document for this issue already exists, default to updating it with verification of its load-bearing claims instead of re-researching from scratch.
...
1. **Resolve issue and read ticket file**:
   ...
   - If it does not exist, create it using `.agents/contracts/engineering/process-artifact.md`
...
6. **Wait for all sub-agents to complete and synthesize findings**:
   ...
   - If the research phase encounters substantive friction (for example wrong assumptions, misleading historical context, helper/tooling friction, or repeated file-discovery dead ends), record it in the process file incident log using `.agents/contracts/engineering/process-artifact.md`
