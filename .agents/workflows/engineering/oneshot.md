...
1. Resolve `ISSUE_NUMBER` (from the argument, or via the auto-selection rules in `.agents/workflows/engineering/ralph_research.md`).
2. Read and follow `.agents/workflows/engineering/core/10-research.md` for the issue.
3. After research completes, the issue is in `Research in Review`. Since the operator explicitly chose the combined flow, advance it and continue:
   ...
   ```
4. Read and follow `.agents/workflows/engineering/core/20-plan.md` for the same issue.
