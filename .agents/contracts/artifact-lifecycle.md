...
- Before cleanup, record the attributable operator decision in evidence using
...
- Primary review documents are written for an operator, not for the workflow.
  `brief`, `business`, `research`, `strategy`, `brand`, `design`, and every
  ...
  Evidence and asset indexes are reference registers rather than linear reading;
  they still keep only current material rows and concise limitations.
...
- Before every pre-development invocation, run the GitHub reconciliation
  ...
  layer for the ledger, state, evidence, and living sources. Treat matching
...
- During initial interview, keep partial facts in the brief, evidence register,
  and decision profile. Do not regenerate all downstream artifacts after every
...
- Before saving an updated document, find every earlier statement about each
  ...
  answer below a contradictory or obsolete answer. Evidence IDs replace repeated
  explanations: state the decision once and link the supporting IDs.
...
- Keep approval state current and compact. The artifact records the current
  ...
  interviews, superseded wording, invalidation logs, profile dispositions, and
  coordinator handoffs belong to Git, evidence, the decision profile, or the
  workflow response rather than the living artifact.
...
- A completed heading or fluent generic prose is not a completed artifact. Every
  material profile row must have the disposition required by its type: factual
  rows answered, approval rows approved, unresolved rows blocked, or inapplicable
  rows justified. An assumption cannot answer an operator-controlled fact.
...
- Presentation source is semantic HTML rendered by presentation-only React
  ...
  project knowledge, evidence, registered brand assets, or editable design
  sources.
- Product catalogs resolve atomically. Empty startup inherits the entire
  ...
  completely. Every active product owns its Product, Website, Marketing
  Creative, and Presentation files in one layer. Never mix product folders or
