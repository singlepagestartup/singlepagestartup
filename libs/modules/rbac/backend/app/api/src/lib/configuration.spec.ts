/**
 * BDD Suite: rbac configuration unit contracts.
 *
...
describe("rbac configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/rbac");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "rbac"');
