...
export class Handler {
  ...
  async execute(c: Context, next: any): Promise<Response> {
    try {
      ...
      if (!data["ecommerce-module"]) {
        throw new Error("No ecommerce-module provided");
      }

      if (!data["ecommerce-module"]["orders"]) {
        throw new Error("No orders provided");
      }

      if (!data["billing-module"]["currency"]) {
        throw new Error("No billing-module currency provided");
      }

      if (!data["ecommerce-module"]["orders"].length) {
        throw new Error("No orders provided");
      }

      if (
        !data["ecommerce-module"]["orders"].every((order: { id: string }) => {
          return order.id;
        })
      ) {
