...
type TProps =
  IParentProps["ISocialModuleProfileFindByIdChatFindByIdProfileFindByIdSkillLinkProps"];
export type IProps = Omit<TProps, "host" | "options"> & {
  reactQueryOptions?: Partial<UseMutationOptions<any, DefaultError, TProps>>;
};
export type IResult =
  IParentResult["ISocialModuleProfileFindByIdChatFindByIdProfileFindByIdSkillLinkResult"];
...
export function action(props: IProps) {
  ...
  return useMutation<IResult, DefaultError, TProps>({
    ...
    mutationFn: (value) =>
      api.socialModuleProfileFindByIdChatFindByIdProfileFindByIdSkillLink({
        ...value,
        host: clientHost,
        options: {
          ...value.options,
