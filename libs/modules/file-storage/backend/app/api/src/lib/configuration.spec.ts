/**
 * BDD Suite: file-storage configuration unit contracts.
 *
...
describe("file-storage configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/file-storage");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "file-storage"');
