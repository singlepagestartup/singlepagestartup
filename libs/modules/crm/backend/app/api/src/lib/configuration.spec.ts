/**
 * BDD Suite: crm configuration unit contracts.
 *
...
describe("crm configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/crm");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "crm"');
