/**
 * BDD Suite: ecommerce configuration unit contracts.
 *
...
describe("ecommerce configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/ecommerce");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "ecommerce"');
