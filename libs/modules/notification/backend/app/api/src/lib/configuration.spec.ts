/**
 * BDD Suite: notification configuration unit contracts.
 *
...
describe("notification configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/notification");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "notification"');
