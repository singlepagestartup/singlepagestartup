/**
 * BDD Suite: telegram configuration unit contracts.
 *
...
describe("telegram configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/telegram");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "telegram"');
