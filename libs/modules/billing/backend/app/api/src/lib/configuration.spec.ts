/**
 * BDD Suite: billing configuration unit contracts.
 *
...
describe("billing configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/billing");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "billing"');
