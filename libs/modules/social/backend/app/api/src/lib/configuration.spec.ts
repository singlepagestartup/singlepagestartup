/**
 * BDD Suite: social configuration unit contracts.
 *
...
describe("social configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/social");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "social"');
