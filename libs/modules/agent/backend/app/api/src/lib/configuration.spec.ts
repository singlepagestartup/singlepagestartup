/**
 * BDD Suite: agent configuration unit contracts.
 *
...
describe("agent configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/agent");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "agent"');
