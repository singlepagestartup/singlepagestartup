/**
 * BDD Suite: startup configuration unit contracts.
 *
...
describe("startup configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/startup");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "startup"');
