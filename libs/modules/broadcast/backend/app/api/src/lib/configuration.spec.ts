/**
 * BDD Suite: broadcast configuration unit contracts.
 *
...
describe("broadcast configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/broadcast");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "broadcast"');
