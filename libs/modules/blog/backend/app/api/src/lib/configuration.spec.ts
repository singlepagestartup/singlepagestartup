/**
 * BDD Suite: blog configuration unit contracts.
 *
...
describe("blog configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/blog");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "blog"');
