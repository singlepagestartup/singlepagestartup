/**
 * BDD Suite: host configuration unit contracts.
 *
...
describe("host configuration unit contracts", () => {
  const moduleRoot = join(process.cwd(), "libs/modules/host");
  ...
  it("keeps module seed metadata anchored to the current module", () => {
    ...
    for (const item of sourceByPath) {
      ...
      expect(item.source).toContain('module: "host"');
