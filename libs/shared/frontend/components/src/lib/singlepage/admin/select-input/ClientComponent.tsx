...
function SelectInputDataLoader<M extends { id?: string }, V>(
  ...
  const baseParams = props.apiProps?.params ?? {};
  const normalizedSearch = debouncedSearchValue.trim();
  const searchField =
    props.searchField ||
    (typeof props.renderField === "string" ? props.renderField : "id");
  ...
  const { data, isLoading, isFetching } = api.find({
    ...childProps.apiProps,
  ...
  const mergedData = useMemo(() => {
    ...
    if (searchByIdEnabled && !shouldPreferIdSearch) {
      (idSearchQuery.data || []).forEach((entity) => {
        if (entity?.id) {
          entities.set(entity.id, entity);
        }
      });
